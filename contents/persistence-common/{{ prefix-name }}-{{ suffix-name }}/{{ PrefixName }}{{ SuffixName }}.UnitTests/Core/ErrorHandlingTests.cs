...
public class ErrorHandlingTests
{
    ...
    public async Task Get{{ PrefixName }}_ShouldReturnNull_WhenEntityNotFound()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<EntityNotFoundException>(() => _service.Get{{ PrefixName }}(id));
    }
    ...
    public async Task Get{{ PrefixName }}_ShouldThrowValidationException_WhenIdIsInvalid()
    {
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<ValidationException>(() => _service.Get{{ PrefixName }}(id));
        exception.ErrorCode.Should().Be("VALIDATION_ERROR");
    ...
    {
        // Arrange
        var request = new Update{{ PrefixName }}Input { Id = Guid.NewGuid().ToString(), Name = "Updated Name" };
        var entityId = Guid.NewGuid();
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<EntityNotFoundException>(() => _service.Update{{ PrefixName }}(request));
    ...
    public async Task Delete{{ PrefixName }}_ShouldThrowEntityNotFoundException_WhenEntityNotFound()
    {
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<EntityNotFoundException>(() => _service.Delete{{ PrefixName }}(id));
        exception.ErrorCode.Should().Be("ENTITY_NOT_FOUND");
        exception.EntityType.Should().Be("{{ PrefixName }}");
        exception.EntityId.Should().Be(entityId.ToString());
    }
    ...
    public async Task Get{{ PrefixName }}s_ShouldThrowValidationException_WhenPaginationIsInvalid()
    {
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<ValidationException>(() => _service.Get{{ PrefixName }}s(request));
        exception.ErrorCode.Should().Be("VALIDATION_ERROR");
